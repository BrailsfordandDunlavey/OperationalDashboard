<?php
	...
	//BillyB insert Company Name
	$username = get_user_meta(($post->post_author), 'user_companyname', true);
...
?>
    ...
    ?>
      ...
      <?php       
		...
		//BillyB add if statement to only show poster's name if the poster is the owner of the Request
		echo '<div class="message_board">';
			echo '<div class="message_board_title">';
			if(($post_au->post_author) == ($row->uid))
				echo sprintf(__('Posted by <b>%s</b> on <b>%s</b>','ProjectTheme'), $username, date_i18n(get_option('date_format'), $now) );
			else	
				echo sprintf(__('Posted by <b>%s</b> on <b>%s</b>','ProjectTheme'), 'Anonymous', date_i18n(get_option('date_format'), $now) );
			echo '</div>';

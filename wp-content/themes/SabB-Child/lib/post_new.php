    	...
    	<div id="content" class="sonita">
            ...
            <div class="my_box3">            
    			...
    			<?php
					...
					echo '<div id="steps">';
						echo '<ul>';
							echo '<li '.($new_Project_step == '1' ? "class='active_step' " : "").'>'.__("Info", 'ProjectTheme').'</li>';
							...
							echo '<li '.($new_Project_step == '4' ? "class='active_step' " : "").'>'.__("Finish", 'ProjectTheme').'</li>';
...
if($new_Project_step == "1")
{
 ...
 <form method="post" action="<?php echo ProjectTheme_post_new_with_pid_stuff_thg($pid, '1');?>">  
    ...
    <?php do_action('ProjectTheme_step1_before_title'); ?>
        ...
        <li>
        	<h3><?php _e('Request: Main Information','ProjectTheme'); ?></h3>
        </li>
        ...
        <?php echo projecttheme_get_post_new_error_thing_display('project_title') ?>
        	...
        	<h2><?php echo __('Request Title', 'ProjectTheme'); ?></h2>
        ...
        <p><textarea rows="6" cols="60" class="full_wdth_me do_input_new description_edit" placeholder="<?php _e('Describe the details of the Request here.  Please be descriptive.  You can also upload a file below','ProjectTheme') ?>"  name="project_description"><?php echo trim($pst); ?></textarea></p>
        </li>
     ...
     <?php do_action('ProjectTheme_step1_before_category'); ?>  
        ...
        <p><input type="text" size="50" class="full_wdth_me do_input_new"  name="project_tags" placeholder="<?php _e('Please separate tags with a comma.','ProjectTheme') ?>" value="<?php echo $project_tags; ?>" /> </p>
        </li>
	  ...
	  ?>
       ...
       <p><input type="text" name="ending" id="ending" class="full_wdth_me do_input_new" value="<?php echo $dt; ?>"  /></p>
		...
		?>
          ...
          <li>
        	<h3><?php _e('Location Information','ProjectTheme'); ?></h3>
        </li>
	   ...
	   ?> 
        ...
        <p><input type="text" size="50" class="full_wdth_me do_input_new" placeholder="<?php _e('eg: Washington, 20006','ProjectTheme'); ?>"  name="project_location_addr" value="<?php echo !isset($_POST['project_location_addr']) ? 
		get_post_meta($pid, 'Location', true) : $_POST['project_location_addr']; ?>" /> </p>
        </li>
        <?php endif; endif; 
		
		//BillyB hide images from Request posting
		?>
        <!--
         <li>
...
myDropzone.on("removedfile", function(file, response) {
	...
	<?php _e('Click the gray area below to add project images. Other files are not accepted. Use the form below.','ProjectTheme') ?>
...
myDropzone.on("removedfile", function(file, response) {
	...
	<?php _e('Click the gray area below to add project files. Images are not accepted.','ProjectTheme') ?>
...
if($new_Project_step == "2")
{
	...
	?>
 	...
 	<ul class="post-new">
      ...
      <h2><?php echo __('Evaluation Criteria','ProjectTheme'); ?>:</h2>
      	<?php /*-------  custom fields  -------- */ ?>
        <?php
		...
		if($show_fields_in_step2 == true):
			...
			$arr = ProjectTheme_get_project_category_fields($catid, $pid);
				...
				for($i=0;$i<count($arr);$i++)
				{
							...
							echo '<h2>'.$arr[$i]['field_name'].$arr[$i][''].'</h2>';
      	...
      	<!-- <li>
        	<h2><?php echo __('Service/Products','ProjectTheme'); ?>:</h2>
        	<p><input type="text" size="35" name="evaluation_criteria_services" value="<?php echo get_user_meta($uid, 'evaluation_criteria_services', true); ?>" class="do_input" /></p>
        </li> -->
        <div>&nbsp;</div>
        <h2><?php echo __('Request Options','ProjectTheme'); ?>:</h2>
        <li>
...
if($new_Project_step == "4")
{
	...
	if($total == 0 && $finalize == true)
	{
		if(get_option('projectTheme_admin_approves_each_project') != 'yes'):
			...
			echo '<tr>';
			echo '<td></td>';
			echo '<td><div class="clear100"></div><a href="'.get_permalink($pid).'" class="go_back_btn">'.__('See your Request','ProjectTheme') .'</a></td>';
			echo '</tr>';	

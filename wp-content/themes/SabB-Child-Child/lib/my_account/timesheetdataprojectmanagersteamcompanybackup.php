...
/////Non-Project Hours/////////////['Brailsford & Dunlavey']
...
echo "<table style=\"table-layout: fixed; width: 100%\"><tr>";
	...
	foreach($result_emp_sup as $keyspheres => $vals_spheres){	
		$team_management_spheres[] = $vals_spheres->user_id;
	...
	foreach($team_management_spheres as $key => $vals){
		...
		$total_numbers_company = total_number_hrs_company($vals, $arr_output, $arr_output_nonprojects, $workingDaysInaMonthmembers);
		...
		//print_r($expectedhour_monthone_sphere);
...
function total_number_hrs_company($user_id, $arr_output, $arr_output_nonprojects){	 
	$s = 0;
	if($_POST['date']){
	...
	for($et=0; $et<=2; $et++){
			...
			foreach($period as $dt) {
				...
				// substract if Saturday or Sunday$user_id_count_arr_company
			...
			echo "<br />";
			echo ($days*8.5);	
			$workingDaysInaMonthmembers[] = ($days*8.5)* $user_id_count_arr_company;
	}
	$d=0;

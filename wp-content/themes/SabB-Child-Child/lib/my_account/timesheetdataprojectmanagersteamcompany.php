...
global $wpdb;
global $first_arr_company;
$first_arr_company = array();

$table = 'timesheets';
$client_table = 'projects';
$table_name = $wpdb->prefix . $table;
$client_table_name = $wpdb->prefix . $client_table; 
$result_company = $wpdb->get_results("SELECT * FROM $table_name INNER JOIN $client_table_name ON ID = project_id
WHERE timesheet_date BETWEEN UNIX_TIMESTAMP('2020-01-01') AND UNIX_TIMESTAMP('2020-04-01')");

//$array = json_decode(json_encode($result), true);
$result_nonproject_company = $wpdb->get_results(
			"SELECT * FROM $table_name
				WHERE timesheet_date BETWEEN UNIX_TIMESTAMP('2020-01-01') AND UNIX_TIMESTAMP('2020-06-01') AND (project_id LIKE '%Vacation%'
...
foreach($result_company as $key=>$arr){
...
/////Non-Project Hours/////////////status = 1
global $arr_output_nonprojects; 
$arr_output_nonprojects = array();
foreach($result_nonproject_company as $key_nonprojects=>$arr_nonprojects){
...
echo "<table style=\"table-layout: fixed; width: 100%\"><tr>";
	...
	$team_management_spheres = array();
	foreach($result_emp_sup as $keyspheres => $vals_spheres){
		$spheres = $vals_spheres->sphere;
		$team_management_spheres[$spheres][] = $vals_spheres->user_id;
		$team_management_position[] = $vals_spheres->position;
		//$team_management_spherest[] = $vals_spheres->position;

	}

	$team_management_spherespop = array_pop($team_management_spheres);
	$multiply_expectHrs = count($team_management_position);
	//echo "<pre>";
	//print_r($team_management_spheres);
	//print_r($team_management_spherest);
	//echo "</pre>";
	$the_sum_total_zero = "";
	$the_sum_total_one = "";
	$the_sum_total_two = "";
	...
	foreach($team_management_spheres as $key => $vals){
		...
		//$team_management_position = $team_management_position;
		//print_r($team_management_spheres[0]);
		$user_id = $vals->user_id;
		$total_numbers_company = total_number_hrs_company($vals, $arr_output, $arr_output_nonprojects, $multiply_expectHrs);
			//echo "<pre>";
			//print_r($total_numbers_company);
			//echo "</pre>";
			$workingDaysInaMonthmembers = $total_numbers_company[2];
			$first_month = 01;
			if(!empty($total_numbers_company[0])){
				$expolode_sum_total_num = explode("/", $total_numbers_company[0]);
				
				foreach($expolode_sum_total_num as $key_explode => $val_explode){							
					if($val_explode){						
						$first_arr_company[$key_explode] = $val_explode;					
					}				
				}					
				$the_sum_total_zero += $first_arr_company[0];
				$the_sum_total_one += $first_arr_company[1];
				$the_sum_total_two += $first_arr_company[2];			
			}
			
			if(!empty($total_numbers_company[1])){
				$expolode_sum_total_numnon = explode("/", $total_numbers_company[1]);		
				foreach($expolode_sum_total_numnon as $key_explodenon => $val_explodenon){						
					if($val_explodenon){					
						$first_arr_companynon[$key_explodenon] = $val_explodenon;					
					}								
				}
				$the_sum_total_zeronon += $first_arr_companynon[0];
				$the_sum_total_onenon += $first_arr_companynon[1];
				$the_sum_total_twonon += $first_arr_companynon[2];				
			}
			//$team_management_position[] = $vals;
			

		$total_expect_count = 254;
		...
		//print_r($added_expected);
		echo "<td><hr>";
		print_r("<a href=/wp-opdash/time-analysis-hours-worked-per-project?user_id=$spherevals>Brailsford & Dunlavey</a>");	
			...
			$c++;
			// 
			myCompanyChart($charpiearr, $chartdivv);
			 
			echo "</td>";

echo "</tr></table>";	
 
function total_number_hrs_company($user_id, $arr_output, $arr_output_nonprojects, $multiply_expectHrs){	 
	$s = 0;
	if($_POST['date']){
	...
	global $user_id_count_arr_company;
	...
	$user_id_count_arr_company = $multiply_expectHrs;
	//$user_id_count_arr_company[] = count($user_id);
	$user_id_count_arr_company_add = $user_id_count_arr_company;
	//print_r($user_id_count_arr_company);
	echo "<br />";
	for($et=0; $et<=2; $et++){
			...
			foreach($period as $dt) {
				...
				// substract if Saturday or Sunday$user_id_count_arr_company$user_id_count_arr_company
			...
			//echo "<br />";
			//echo ($days*8.5);	
			$workingDaysInaMonthmembers[] = ($days*8.5)* 126;
	}
	$d=0;
...
function myCompanyChart($charpiearr, $chartdivv){
	?>	
	<!-- Styles -->
	<style>
	<?php echo "#".$chartdivv; ?> {

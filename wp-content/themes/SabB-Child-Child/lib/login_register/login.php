			...
			case 'login' : 
			  ...
			  //Any differences will be noted with end of line comments. 
				...
				if($_POST)
				{
					...
					{
						$user_login_a = explode('@',$_POST['log']);
						$user_login = $user_login_a[0];
						
						//$name_query = $wpdb->get_results($wpdb->prepare("select user_login from ".$wpdb->prefix."users
						//	where user_email=%s",$_POST['log']));
						//if(count($name_query)==1)
						//{
						//	$user_login = $name_query[0]->user_login;
						//}
					}
					else
					{
				...
				<?php echo "Keep me logged in<br/><br/>"; ?>
				</p> 
                <?php do_action('login_form'); ?>
				<p><label>&nbsp;</label>
				<input type="submit" class="my-buttons" name="submits" id="submits" value="<?php _e('Sign in',$current_theme_locale_name); ?>" tabindex="4" />
				<input type="hidden" name="redirect_to" value="<?php echo wp_specialchars($redirect_to); ?>" />
				</p>	
                </form>
                <ul id="logins">
					<li><a class="my-buttons" href="<?php bloginfo('home'); ?>/" 
						title="<?php _e('Are you lost?',$current_theme_locale_name) ?>">&laquo; <?php _e('Home',$current_theme_locale_name) ?></a></li>
					<li><a class="my-buttons" href="<?php bloginfo('wpurl'); ?>/wp-login.php?action=lostpassword" 
